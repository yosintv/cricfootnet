              ...
              <div className="space-y-3">
                {dayData.matches.map((match) => (
                  <Link
                    ...
                    className="block bg-slate-900/50 rounded-lg p-4 hover:bg-slate-900/70 transition-all border border-white/5 hover:border-blue-400/50 cursor-pointer transform hover:scale-[1.02]"
                    data-testid={`match-${match.match_id}`}
                  >
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                      <div className="flex-1">
                        <div className="text-white font-semibold mb-1 hover:text-blue-300 transition-colors">
                          {match.fixture}
                        </div>
                        <div className="text-blue-300 text-sm">{match.league}</div>
                        {match.venue && (
                          <div className="text-blue-200/60 text-xs mt-1">{match.venue}</div>
                        )}
                      </div>
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {match.tv_channels && match.tv_channels.length > 3 && (
                            <span className="inline-block bg-purple-600/30 text-purple-200 text-xs px-2 py-1 rounded">
                              +{match.tv_channels.length - 3} more
                            </span>
                          )}
                        </div>
                      </div>
